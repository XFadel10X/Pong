...
function movePaddle(e) {
    if (e.key === 'ArrowUp') {
        ...
        paddle2Y = Math.min(paddle2Y + paddleSpeed, pongGame.clientHeight - paddle2.clientHeight);
    }
    
    if (e.key === 'z') {
        paddle1Y = Math.max(paddle1Y - paddleSpeed, 0);
    } else if (e.key === 's') {
        paddle1Y = Math.min(paddle1Y + paddleSpeed, pongGame.clientHeight - paddle1.clientHeight);
    }
...
function moveBall() {
    ...
    if (ballX <= paddle1.clientWidth) {
        if (ballY + ball.clientHeight >= paddle1Y && ballY <= paddle1Y + paddle1.clientHeight) {
    ...
    if (ballX + ball.clientWidth >= pongGame.clientWidth - paddle2.clientWidth) {
        if (ballY + ball.clientHeight >= paddle2Y && ballY <= paddle2Y + paddle2.clientHeight) {
            ballSpeedX = -ballSpeedX;
            increaseBallSpeed();
        } else {
            score1++;
